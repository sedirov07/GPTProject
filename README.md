...
Программа использует библиотеку Streamlit для построения интерфейса и функцию `get_answer` из модуля `model` для выполнения перевода.

## Описание работы программы с использованием API

Это приложение представляет собой API с использованием STREAMLIT для ответа с помощью функции `get_answer`. 
